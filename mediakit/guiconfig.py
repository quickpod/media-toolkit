r"""Tiny JSON-backed config for the Media Toolkit GUI.

Stores the chosen theme ("light"/"dark") and a short recent-files list.  On
Windows the file lives at ``%LOCALAPPDATA%\MediaToolkit\config.json``; elsewhere
it falls back to ``~/.mediatoolkit/config.json``.  Every function is defensive:
a corrupt or unreadable config must never stop the app from starting.
"""
...
def config_dir():
    r"""``%LOCALAPPDATA%\MediaToolkit`` on Windows, ``~/.mediatoolkit`` else."""
